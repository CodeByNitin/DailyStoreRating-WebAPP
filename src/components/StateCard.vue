...
<div v-if="showing" class="card"
			...
			<div class="stars">
                <v-btn fab dark color="red darken-1" @click="rateStore(storeId, 4)">
        			<v-icon dark>whatshot</v-icon>
      			</v-btn>
				<v-btn fab dark color="pink" @click="rateStore(storeId, 5)">
        			<v-icon dark>favorite</v-icon>
      			</v-btn>
				<v-btn fab dark color="blue darken-1" @click="rateStore(storeId, 2)">
        			<i class="material-icons">thumb_up</i>
      			</v-btn>
				<v-btn fab dark color="red accent-4" @click="rateStore(storeId, 0)">
...
export default {
  ...
  data: function() {
    return {
      ...
      threshold: window.innerWidth / 20, // Breakpoint distance to approve/reject a card
  ...
  methods: {
    rateStore(storeId, emojiValue) {
      ...
      httpClient({
        method: "POST",
        url: process.env.API_BASE + "/masterRating",
        data: requestBody
      }).then(res => {
        self.$emit("draggedThreshold", true);
        self.icon.opacity = 1;
        self.$emit("draggedEnded");
        self.$emit("updateSnackbar", true);
      });
    }
  },
  computed: {
